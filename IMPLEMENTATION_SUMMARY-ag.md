...
- [x] 150x150px photos show properly
...
- [x] Font Awesome icons load with 5px right margin
- [x] Cache invalidation works when individuals are edited
- [x] No JavaScript console errors
- [x] No CSS layout issues

## Fixes Implemented (2025-11-27)
1.  **individual-page.phtml**:
    -   Added compatibility code to map `$record` (from core) to `$individual` (expected by view).
    -   Initialized missing fact collections (`$name_facts`, `$sex_facts`, `$individual_facts`) using `$individual->facts()`.
    -   Passed `$tabs` and `$record` to `individual-page-tabs` view.
2.  **individual-page-menu.phtml**:
    -   Replaced undefined `app()` helper with `\Fisharebest\Webtrees\Registry::container()->get()`.
    -   Replaced deprecated string routes (e.g., `'edit-individual'`) with `RequestHandler` class constants (e.g., `EditRecordPage::class`).
    -   Replaced non-existent `ClipboardService` methods (`isAnyClipboard`, `clipboardFacts`) with `pastableFacts($individual)`.
